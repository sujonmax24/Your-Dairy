...
@Composable
fun DiaryEditorScreen(note: NoteEntity?, folders: List<FolderEntity>, onBack: () -> Unit, onSave: (NoteEntity) -> Unit) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = {
                        onSave((note ?: NoteEntity()).copy(title = title.trim(), content = body.text, tags = tags.trim(), folderId = folderId))
        ...
        Column(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp, vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            OutlinedTextField(value = title, onValueChange = { title = it }, modifier = Modifier.fillMaxWidth(), singleLine = true, label = { Text("Title") })
            ...
            HorizontalDivider()
            BasicTextField(value = body, onValueChange = { body = it }, modifier = Modifier.fillMaxWidth().weight(1f), textStyle = MaterialTheme.typography.bodyLarge.copy(color = MaterialTheme.colorScheme.onBackground), decorationBox = { innerTextField ->
                if (body.text.isEmpty()) Text("Write your memory... Select text and use the toolbar above.", style = MaterialTheme.typography.bodyLarge, color = MaterialTheme.colorScheme.onSurfaceVariant)
                innerTextField()
            })
        }
    }
    ...
    if (showFolderDialog) {
        AlertDialog(onDismissRequest = { showFolderDialog = false }, title = { Text("Choose folder") }, text = {
    ...
    if (showTableDialog) {
        TableInsertDialog(onDismiss = { showTableDialog = false }, onInsert = { rows, columns -> insertAtSelection(createMarkdownTable(rows, columns)); showTableDialog = false })
    }
}

@Composable
private fun TableInsertDialog(onDismiss: () -> Unit, onInsert: (Int, Int) -> Unit) {
    var rows by rememberSaveable { mutableIntStateOf(2) }
    var columns by rememberSaveable { mutableIntStateOf(2) }
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Insert table") }, text = { Column(verticalArrangement = Arrangement.spacedBy(12.dp)) { Text("Choose rows and columns"); Stepper("Rows", rows, 1, 20) { rows = it }; Stepper("Columns", columns, 1, 10) { columns = it } } }, confirmButton = { Button(onClick = { onInsert(rows, columns) }) { Text("Insert") } }, dismissButton = { TextButton(onClick = onDismiss) { Text("Cancel") } })
}

@Composable
private fun Stepper(label: String, value: Int, min: Int, max: Int, onChange: (Int) -> Unit) {
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("$label: $value"); Row { TextButton(onClick = { if (value > min) onChange(value - 1) }) { Text("−") }; TextButton(onClick = { if (value < max) onChange(value + 1) }) { Text("+") } } }
}
...
private fun createMarkdownTable(rows: Int, columns: Int): String {
    ...
    val data = (1 until rows).joinToString("\n") { row -> (1..columns).joinToString(" | ", prefix = "| ", postfix = " |") { "Row $row" } }
